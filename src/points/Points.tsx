...
import { createNamespace, pointSecond, toImg } from '../utils';
...
const Points: FC<PointsProps> = (props) => {
  ...
  const onClick = async (e: any) => {
    if (points.length < CHECK_WORD) {
      ...
      if (pointList.length === CHECK_WORD) {
        ...
        const validate = await onValid(JSON.stringify(pointList), pointSecond(captcha, pointList));
  ...
  return (
    <div className={classNames(className, bem())}>
      <div className={classNames(bem('title'))}>请完成安全验证</div>
